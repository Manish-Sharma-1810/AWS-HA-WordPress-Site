...
# Function to replace special characters with underscore (_)
clean_string() {
    local input_string="$1"
    local cleaned_string
    # Replace all non-alphanumeric characters with an underscore
    cleaned_string=$(echo "$input_string" | sed 's/[^[:alnum:]]/_/g')
    echo "$cleaned_string"
}
...
DEFAULT_MYSQL_WP_DATABASE="wp_db"
...
# Create a new MySQL user (wordpress) for remote access and grant Grant the necessary permissions on the wordpress_db
log "Creating a new MySQL user for remote access for wordpress site and granting necessary privileges on the wordpress_db."
mysql -u root -p"${MYSQL_ROOT_PASSWORD}" <<EOF
CREATE USER '${MYSQL_WP_USER}'@'%' IDENTIFIED BY '${MYSQL_WP_USER_PASSWORD}';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, INDEX, LOCK TABLES ON ${MYSQL_WP_DATABASE}.* TO '${MYSQL_WP_USER}'@'%';
FLUSH PRIVILEGES;
EOF
...
while IFS= read -r line; do
    ...
    aws ssm put-parameter --name "$parameter_name" --value "$cleaned_value" --type SecureString --overwrite
done <<< "$response"
